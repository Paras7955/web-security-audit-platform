...
def generate_reports(scan_id: str, db: Session = Depends(get_db)) -> list[ReportArtifactRead]:
    try:
...
def list_reports(scan_id: str, db: Session = Depends(get_db)) -> list[ReportArtifactRead]:
    try:
        artifacts = list_report_artifacts(db, scan_id=scan_id)
    except ReportGenerationError as exc:
        raise report_error(exc) from exc
    return [to_report_read(artifact) for artifact in artifacts]
...
def view_report(report_id: str, db: Session = Depends(get_db)) -> Response:
    artifact = db.get(ReportArtifact, report_id)
    if artifact is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Report not found.")
    try:
        content = read_report_artifact(db, artifact, artifact_root=settings.artifact_root)
    except ReportGenerationError as exc:
        raise report_error(exc) from exc
...
def download_report(report_id: str, db: Session = Depends(get_db)) -> PlainTextResponse:
    artifact = db.get(ReportArtifact, report_id)
    if artifact is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Report not found.")
    try:
